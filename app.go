...
func (a *App) OpenDocument() (*models.Document, error) {
	path, err := runtime.OpenFileDialog(a.ctx, runtime.OpenDialogOptions{
		...
		Filters: []runtime.FileFilter{
			{DisplayName: "文档 (*.md; *.txt)", Pattern: "*.md;*.txt"},
...
// loadDocument 读取文件并登记入库。UTF-8 编码，容忍 BOM。
...
func (a *App) ImportAnnotations(docID string) (*ImportResult, error) {
	...
	existing, err := a.store.ExistingDedupeKeys(docID)
	if err != nil {
		return nil, err
	}
	fresh := pack.MergeImport(p, docID, existing, time.Now().UnixMilli())
	for i := range fresh {
		if _, err := a.SaveAnnotation(fresh[i]); err != nil {
			return nil, err
		}
	}
	return &ImportResult{
		Imported:     len(fresh),
		Skipped:      len(p.Annotations) - len(fresh),
